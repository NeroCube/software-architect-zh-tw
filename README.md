...
* **分析和理解應用模式**: 看一下當前的任何框架，例如Angular。 你可以在實踐中研究很多模式，例如“可視化”。 嘗試了解它如何在框架中應用，為什麼要這樣做。 而且，如果你真的很專心，請更深入地研究程式碼並了解其實現方式。
...
## (5) 文件
架構文檔有時或多或少地重要。 重要文檔是例如架構決策或程式碼準則。 編程開始之前通常需要初始文檔，並且需要不斷完善。 其他文檔可以自動生成，因為程式碼也可以是文檔，例如 UML類圖。

* **簡潔程式碼**: 如果做對的話，程式碼是最好的文檔。 一個好的架構師應該能夠區分好的程式碼和壞的程式碼。 Robert C. Martin 所著的 "Clean Code" 一書是了解更多關於好壞程式碼的寶貴資源。
* **盡可能生成文檔**: 系統變化迅速，很難更新文檔。 無論是有關 API 還是 CMDB（配置管理數據庫）形式的系統格局： 基礎信息經常變化太快而無法手動更新相應的文檔。 示例：對於API，如果你是模型驅動的，則可以基於定義文件自動生成文檔或直接從源程式碼中獲取。 為此，存在許多工具，我認為 Swagger 和 RAML 是學習更多內容的一個很好的起點。
* **盡可能少**: 無論你需要記錄什麼文件（例如決策文件），都應嘗試一次只關註一件事，並且僅包含關於這件事的必要信息。 大量的文檔很難閱讀和理解。 附加信息應存儲在附錄中。 特別是對於決策文件，講一個有說服力的故事而不是僅僅進行大量爭論，其更為重要。 此外，這為你和你的同事節省了很多時間，而後者需要閱讀。 看看你過去所做的一些文檔（源程式碼，模型，決策文件等）並問自己以下問題："是否包含所有必要的信息才能理解它？"，"確實需要哪些信息，可以省略哪些信息？" 和"文檔中是否有紅線？"。
...
* **建立實踐社區（CoP）**: 在一個共同興趣小組之間交流經驗和知識有助於分發思想和標準化方法。 例如，你可以每三個月左右將所有JavaScript開發人員和架構師聚集在一個房間中，討論過去和當前的挑戰以及如何解決它們或採用新的方法論和方法。 建築師可以共享，討論和調整其願景，開發人員可以共享經驗並向同行學習。 這樣的回合不僅可以為企業帶來極大的好處，而且對個人本身也非常有利，因為它有助於建立更強大的網絡並傳播思想。還可以查看共體時間 SAFe 框架中的文章，該框架在敏捷環境中解釋了CoP概念。
...
* **激勵並說服**: 公司如何說服你購買產品？ 他們證明了它的價值和好處。 但不僅僅是5點。 他們包裝得很好，並使其盡可能容易消化。
  ...
  2. 影片展示：取代 "無聊的幻燈片" ，你還可以展示一個影片，該影片可以演示你的想法或至少是方向。
但是請不要過度營銷：從長遠來看，內容為王。 如果你的話不正確，從長遠來看，這將損害你的聲譽。
* **為你的想法而奮鬥並堅持不懈**: 人們有時不喜歡你的想法，或者他們太懶惰而無法遵循它們。 如果你真的對自己的想法深信不疑，則應不斷追求並"奮鬥"。有時這是必要的。 具有長期目標的架構決策通常不是最容易的：開發人員不喜歡它們，因為它們的開發更加複雜。 經理們不喜歡它們，因為它們在短期內更昂貴。 這是你要堅持不懈並進行談判的工作。
